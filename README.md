...
1. 상단 **내 파트** 드롭다운에서 본인 파트를 선택하고, **작성자·비밀번호**를 입력
...
5. 페이지 하단 **CSV 내보내기**로 엑셀에서 열어 정리 가능(한글 깨짐 방지 처리됨)

## 비밀번호 보호 / 관리자 권한

- **작성자 비밀번호:** 셀을 저장할 때 상단에 입력한 비밀번호로 그 항목이 잠깁니다.
  이후 **수정·삭제하려면 같은 비밀번호**가 필요합니다. (다른 사람의 임의 삭제 방지)
- **관리자:** 비밀번호 칸에 **`1657`** 을 입력하면 관리자 모드가 켜져,
  **모든 파트의 모든 셀을 비밀번호 없이 입력·수정·삭제**할 수 있습니다.
  (관리자 비밀번호는 `js/config.js`의 `ADMIN_PASSWORD`에서 변경 가능)
- 비밀번호는 평문이 아니라 **SHA-256 해시**로만 저장됩니다.

> ⚠️ 이 비밀번호 보호는 **클라이언트(화면) 단의 실수·임의 삭제 방지용**입니다.
> Realtime Database 규칙이 공개(read/write)인 상태에서는 기술적으로 우회가 가능하므로,
> 엄격한 접근 통제가 필요하면 Firebase 인증(Auth) 기반 규칙 적용을 권장합니다. 필요 시 요청해 주세요.
